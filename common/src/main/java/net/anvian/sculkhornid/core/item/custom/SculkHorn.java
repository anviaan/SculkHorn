...
public abstract class SculkHorn extends Item {
    ...
    protected void applyCooldownToBothHorns(Player player) {
        player.getCooldowns().addCooldown(ModItems.SCULKHORN.getDefaultInstance(), TimeUtil.secondsToTicks((float) this.config.areaCooldown));
        player.getCooldowns().addCooldown(ModItems.SCULKHORN_SONICBOOM.getDefaultInstance(), TimeUtil.secondsToTicks((float) this.config.distanceCooldown));
    }
}
